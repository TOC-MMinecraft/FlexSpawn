...
`FlexSpawn` 是一个面向 `Paper 1.21` 的出生点控制插件。

它会按场景接管玩家的出生位置，并将玩家档案与可选的个人存档点持久化到 `players.yml`。
...
字段说明：
- `enabled`：是否为首次加入启用专属出生点逻辑。
- `group`：目标坐标组名，对应 `groups.yml` 顶层节点。
- `point`：可选，指定固定坐标点；留空时按权限和权重随机。
